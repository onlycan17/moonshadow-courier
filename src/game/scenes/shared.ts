...
export function getSlotStates(repository: ProfileRepository): [SlotState, SlotState, SlotState] {
  return [1, 2, 3].map((slot) => getSlotState(repository, slot as SlotNumber)) as [SlotState, SlotState, SlotState];
}

export function hasWritableEmptySlot(slotStates: readonly SlotState[]): boolean {
...
function getSlotState(repository: ProfileRepository, slot: SlotNumber): SlotState {
